...
if __name__== "__main__":
    ...
    # input must be an integer between 3 and 10 inclusive
    size = int(sys.argv[1])
    ...
    print('Starting timing on the cluster algorithm')
    start = time.time()
    ...
    # for each core, create a new job which runs the distance function over a segment of the set of routes
    for split in range(cores):
        return_minimum_distance_a, return_minimum_order_a, return_maximum_distance_a, return_maximum_order_a = distance(split_array1[split], distance_dictionary, size)
    ...
    # start the timer for the individual process
    print('Starting timing on an single process')
    start = time.time()
    
    # calculate the minimum across the whole permutation
